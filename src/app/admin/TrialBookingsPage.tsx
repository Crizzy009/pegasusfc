...
import type { TrialBooking } from "../content/types";
import { Eye, Loader2, Trash2 } from "lucide-react";

const LAST_SEEN_KEY = "trial_bookings_last_seen";

export function TrialBookingsPage() {
  const [searchParams] = useSearchParams();
  const { items, loading, updateItem, deleteItem } = useAdminCRUD<TrialBooking>("trialBooking");
  const [query, setQuery] = useState("");
  const [open, setOpen] = useState(false);
  const [activeId, setActiveId] = useState<string | null>(null);
  const [form, setForm] = useState<TrialBooking | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((it) => {
      ...
      return (
        ...
        String(d.ageGroup ?? "").toLowerCase().includes(q) ||
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="w-[320px] max-w-full">
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search name, guardian, age group, phone, email..." />
        </div>
      </CardHeader>
      <CardContent>
        <Table>
          ...
          <TableBody>
            {filtered.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.data.age}</TableCell>
                <TableCell>{item.data.ageGroup}</TableCell>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Input value={form.ageGroup} readOnly />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
